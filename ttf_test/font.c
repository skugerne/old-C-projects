...
#ifdef FONT_DEBUG
bool g_borders, g_stepUp;
textjusty_t g_textVert;
#endif
...
void initText(){
  ...
  loadFont("fonts/chowfun.ttf","chow");
  loadFont("fonts/dirtydozen.ttf","dirty");
...
void drawGlyph(unsigned char index){
  ...
  if( g_borders ){
    ...
    glBegin(GL_LINE_LOOP);
      glVertex2f(curX, curY);
      glVertex2f(curX + curGlyphs->glyph_advance[index], curY);
      glVertex2f(curX + curGlyphs->glyph_advance[index], curY + curGlyphs->font_h);
      glVertex2f(curX, curY + curGlyphs->font_h);
    glEnd();
...
void renderText(char *str){
  ...
  #ifdef FONT_DEBUG
  // draw an X over our starting spot
  if( g_borders ){
    glColor4f(0.0,1.0,0.0,0.7);
    glBegin(GL_LINES);
      glVertex2f(curX-40,curY-40);
      glVertex2f(curX+40,curY+40);
      glVertex2f(curX-40,curY+40);
      glVertex2f(curX+40,curY-40);
    glEnd();
